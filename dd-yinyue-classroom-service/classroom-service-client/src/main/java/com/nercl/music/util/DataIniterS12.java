...
public class DataIniterS12 {
	...
	private String initBook() {
		Book book = new Book();
		book.setTitle("义务教育教科书音乐 一年级上册");
		book.setPublishHouse("江苏凤凰少年儿童出版社");
		book.setIntro("义务教育教科书音乐  一年级 上册，江苏凤凰少年儿童出版社出版发行");
		book.setIsbn("ISBN 978-7-5346-6434-2");
		book.setVersion(VersionDesc.SU_JIAO_BAN);
		book.setIsFirstVolume(true);
		Grade grade = gradeService.findByCode("002-1");
		if (null != grade) {
	...
	private void initChapters(String bookId) {
		...
		child2.setTeachForm(TeachForm.LING_TING_S);
		child2.setTitle("快乐小舞曲");
		child2.setOrderBy(2);
		...
		child3.setTeachForm(TeachForm.YAN_CHANG_S);
		child3.setTitle("快乐的音乐娃");
		child3.setOrderBy(3);
		...
		child4.setTeachForm(TeachForm.YAN_CHANG_S);
		child4.setTitle("小手拉小手");
		child4.setOrderBy(4);
		...
		child5.setTeachForm(TeachForm.YAN_ZOU_S);
		child5.setTitle("儿歌诵读");
		child5.setOrderBy(5);
		...
		child6.setTeachForm(TeachForm.YAN_ZOU_S);
		child6.setTitle("律动（上学路上）");
		child6.setOrderBy(6);
		chapterService.save(child6);

		Chapter parent2 = new Chapter();
		parent2.setBookId(bookId);
		parent2.setTitle("听，谁在唱歌");
