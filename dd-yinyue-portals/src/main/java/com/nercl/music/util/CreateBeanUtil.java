/*
 * 此类主要用来根据输入的bean名称，dao，service路径,生成与bena相关的,entity、dao、doaImpl、service、serviceImpl五个类，因为这五个类的内容比较固定，避免每次手动书写
 */
...
public class CreateBeanUtil {
	...
	public static void createBean(String beanName, String beanPath, String daoPath, String servicePath) {
		...
		File bean = new File(root + File.separator + replaceStr(com) + File.separator + replaceStr(beanPath)
				+ File.separator + beanName + ".java");
		File dao = new File(root + File.separator + replaceStr(com) + File.separator + replaceStr(daoPath)
				+ File.separator + beanName + "Dao.java");
		File daoImpl = new File(root + File.separator + replaceStr(com) + File.separator + replaceStr(daoPath)
				+ File.separator + beanName + "DaoImpl.java");
		File service = new File(root + File.separator + replaceStr(com) + File.separator + replaceStr(servicePath)
				+ File.separator + beanName + "Service.java");
		File serviceImpl = new File(root + File.separator + replaceStr(com) + File.separator + replaceStr(servicePath)
				+ File.separator + beanName + "ServiceImpl.java");
	...
	private static void createServiceImpl(String beanName, String beanPath, String servicePath, File bean,
			File serviceImpl) throws IOException {
	...
	private static void createService(String beanName, String beanPath, String servicePath, File bean, File service)
			throws IOException {
	...
	private static void createDaoImpl(String beanName, String beanPath, String daoPath, File bean, File daoImpl)
			throws IOException {
		...
		writeFileContent(daoImpl, "public class " + beanName + "DaoImpl extends AbstractBaseDaoImpl<" + beanName
				+ ", String> implements " + beanName + "Dao {");
		writeFileContent(daoImpl, lineSeparator); // 换行
		writeFileContent(daoImpl, "}");
	}

	private static void createDao(String beanName, String beanPath, String daoPath, File bean, File dao)
			throws IOException {
	...
	public static void main(String[] args) {
		createBean("Transaction", "com.nercl.music.cloud.entity.alipay", "com.nercl.music.cloud.dao", "com.nercl.music.cloud.service");
	}
}
