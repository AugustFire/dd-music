...
public class StaffUtil {
	...
	@Value("${music_exercise.looksing.data.exerciser}")
	private String exerciserLooksingDataPath;

	@Value("${music_exercise.looksing.data.curve}")
	private String looksingDataCurvePath;

	@Value("${music_exercise.exerciser.photo}")
	private String exerciserPhotoPath;

	@Value("${music_exercise.c_sharp_exe}")
	...
	public int getLookSingScore(String standarAnswerPath, String audioPath, String needScore) {
		int result = 0;
		Process process = null;
		File exeFile = null;
		try {
			...
			process = Runtime.getRuntime()
			        .exec(new String[] { exeFile.getPath(), standarAnswerPath, audioPath, needScore, " ", " " });
			process.waitFor();
			process.waitFor();
		} catch (Exception ex) {
			ex.printStackTrace();
