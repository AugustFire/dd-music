...
/*搜索考试和考卷条件联动*/
		...
		$(this).find("dd").click(function(){			
			...
			if("0"!=examId){
				...
				$.getJSON(herf, function(json) {
					var n = json.length;
					var html = "";
					$("#examPaperSelect").html("");
					$("#examPaperSelect").siblings().find(".layui-anim").html("");
					for(var i=0;i<n;i++){
						var html = "<dd lay-value='"+ json[i].id +"'>"+json[i].title+"</dd>";
						$("#examPaperSelect").siblings().find(".layui-anim").append(html);
						$("#examPaperSelect").siblings().find(".layui-anim").find("dd").click(function(){
	...
	$("body").on("click",".layui-input-inline:eq(2)",function(){
		...
		list.click(function(){
			...
			if("0"!=year){
				...
				$.getJSON(herf, function(json) {
					...
					$("#examPaperSelect").html("");
					$("#examPaperSelect").siblings().find(".layui-anim").html("");					
					$("#examPaperSelect").siblings().find(".layui-select-title").find("input").val("");
					$("#examPaperSelect").siblings().find(".layui-select-title").find("input").attr("placeholder","");
					var n = json.length;
					for(var i=0;i<n;i++){
						var html = "<dd lay-value='"+ json[i].id +"'>"+json[i].title+"</dd>";
	...
	var idArray = [];
	var flag = 0;
	$("#examPaperSelect").find("option").each(function(){
		var type =$(this).attr("data-type");
		var id =$(this).attr("value");
		typeArray.push(type);
		idArray.push(id,type);
	});



	/*搜索年度和考试条件联动*/
	$("body").on("click",".layui-input-inline:eq(4)",function(){
		var option = $(this).find("#examPaperSelect").find("option").length;
		var list = $(this).find(".layui-form-select").find("dd");
		var $this = $(this);
		list.click(function(){
			$(this).addClass("layui-this").siblings().removeClass("layui-this");
			var text = $(this).text();
			$(this).parent().siblings(".layui-select-title").find("input").attr("placeholder",text);
			$(this).parent().siblings(".layui-select-title").find("input").attr("value",text);
			/*搜索联动*/
			var type=$(this).index();
			if(typeArray[type]==1){
				$("#question-types").find("p").show();
				$("#question-types").find("p:eq(3)").hide();
				$("#question-types").find("p:eq(2)").find("input").prop("checked",false);
			}
			if(typeArray[type]==3){
				$("#question-types").find("p").hide();
				$("#question-types").find("p:eq(2)").show();
				$("#question-types").find("p:eq(2)").find("input").prop("checked",true);
			}
			if(typeArray[type]==2){
				$("#question-types").find("p").hide();
				$("#question-types").find("p:eq(3)").show();
				$("#question-types").find("p:eq(3)").find("input").prop("checked",true);
			}
	...
	/*获取到url中的type=后面的值*/
	$(".search-form").find("input[type=radio]").each(function(){
		var name = $(this).attr("name");
		var type =getQueryString(name);
		var title = "exam_paper_id";
		var titleId = getQueryString(title);
		var flag = idArray.indexOf(titleId)+1;
		var newType = idArray[flag];
		var id = $("#examPaperSelect").siblings(".layui-form-select").find("dd").filter(".layui-this").attr("lay-value");
		if(newType==1||typeArray[0]==1){
			$("#question-types").find("p").show();
			$("#question-types").find("p:eq(3)").hide();
		}
		if(newType==3||typeArray[0]==3){
			$("#question-types").find("p").hide();
			$("#question-types").find("p:eq(2)").show();
		}
		if(newType==2||typeArray[0]==2){
			$("#question-types").find("p").hide();
			$("#question-types").find("p:eq(3)").show();	
		}
